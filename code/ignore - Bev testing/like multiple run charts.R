...
max_plots_gest_by_BAPM <- 2

gest_by_BAPM_plotListNames = c("between 34 and 36 weeks (inclusive)", # late pre-term
                               "between 37 and 42 weeks (inclusive)" # term/post-term
                               ) 

y_max_percentage <- reactiveVal(0) # initialise y_max_gestation
...
gest_by_BAPM_runchart_data <- reactive ({
  ...
  gest_by_BAPM_runchart_data <- filter(runchart_dataframe,
                                     measure == "BAPM GEST+BAPM" &
                                       measure_cat %in% gest_by_BAPM_plotListNames &
